...
--1. Display Total Sales Amount by Region.
	SELECT 
		REGION , SUM(SALES_AMOUNT) AS TOTAL_AMOUNT
		FROM SALES_DATA
		GROUP BY REGION
--2. Display Average Sales Amount by Product
	SELECT 
		PRODUCT, AVG(SALES_AMOUNT) AS AVG_AMOUNT
		FROM SALES_DATA
		GROUP BY PRODUCT
--3. Display Maximum Sales Amount by Year
	SELECT 
		YEAR, MAX(SALES_AMOUNT) AS MAX_AMOUNT
		FROM SALES_DATA
		GROUP BY YEAR
--4. Display Minimum Sales Amount by Region and Year
	SELECT 
		YEAR,REGION,MIN(SALES_AMOUNT) AS MIN_AMOUNT 
		FROM SALES_DATA
		GROUP BY YEAR,REGION
--5. Count of Products Sold by Region
	SELECT 
		REGION,COUNT(REGION) AS REGION_COUNT
		FROM SALES_DATA
		GROUP BY REGION
--6. Display Sales Amount by Year and Product
	SELECT 
		YEAR,PRODUCT,SUM(SALES_AMOUNT) AS TOTAL_SALES
		FROM SALES_DATA
...
--10. Display Regions with at Least 3 Distinct Products Sold.
	SELECT 
		REGION,COUNT(PRODUCT) AS COUNT_PRODUCT
		FROM SALES_DATA
		GROUP BY REGION
...
--1. Display Count of Orders by Year and Region, Sorted by Year and Region
	SELECT 
		YEAR,REGION,COUNT(SALES_AMOUNT) AS COUNT_AMOUNT
		FROM SALES_DATA
		GROUP BY YEAR,REGION
		ORDER BY YEAR,REGION
--2. Display Regions with Maximum Sales Amount Exceeding 1000 in Any Year, Sorted by Region
	SELECT
		REGION,MAX(SALES_AMOUNT) AS MAX_SALES
		FROM SALES_DATA
		GROUP BY REGION
		ORDER BY REGION
...
--4. Display Top 3 Regions by Total Sales Amount in Year 2024
	SELECT 
		REGION,SUM(SALES_AMOUNT) AS TOTAL_AMOUNT_2024
		FROM SALES_DATA
		WHERE YEAR = 2024
		GROUP BY REGION
...
--2. Display Years with More Than 5 Orders from Each Region
	SELECT 
		YEAR,COUNT(PRODUCT) AS ORDERS
		FROM SALES_DATA
		GROUP BY YEAR
		HAVING COUNT(PRODUCT) > 5
--3. Display Regions with Average Sales Amount Above 1500 in Year 2023 sort by amount in descending.
	SELECT 
		REGION,AVG(SALES_AMOUNT) AS AVG_SALES_2023
		FROM SALES_DATA
		WHERE YEAR = 2023
		GROUP BY REGION
